...
def create_node(df, mode, sub=None):
    ...
    # add
        ...
        # node = df[sub].values.tolist()
        nrow, ncol = df.shape
        feature_ind = np.array(range(ncol))
        feature_node = np.zeros((ncol, ncol + sub.shape[1]))
        feature_node[np.arange(ncol), feature_ind + sub.shape[1]] = 1
        sample_node = np.zeros((nrow, ncol + sub.shape[1]))
        for i in range(sub.shape[1]):
            sample_node[:, i] = sub.iloc[:, i]
        node = sample_node.tolist() + feature_node.tolist()
    return node
...
def create_edge(df):
    ...
    for x in range(n_row):
        edge_start = edge_start + [x] * n_col # obj (# features)
        edge_end = edge_end + list(n_row+np.arange(n_col)) # att    
    edge_start_new = edge_start + edge_end    # like 雙向的
...
def create_edge_attr(df):
    ...
    edge_attr = edge_attr + edge_attr    # 雙向
    return edge_attr

def get_data(df_X, df_y, node_mode, train_edge_prob, split_sample_ratio, split_by, train_y_prob, args, seed=0, normalize=True):
    if len(df_y.shape)==1:
        df_y = df_y.to_numpy()
    ...
    if args.data_type == 'cls':
        df_y = preprocessing.LabelEncoder().fit(df_y).transform(df_y)

    # add
    if normalize:
        x = df_X[args.continuous].values
        min_max_scaler = preprocessing.MinMaxScaler()
        x_scaled = min_max_scaler.fit_transform(x)
        df_X[args.continuous] = x_scaled

    # add
    if node_mode == 3:
        node_feat = args.continuous
        init_feat = args.discrete   
    elif node_mode == 4:
        node_feat = args.discrete
        init_feat = args.continuous 
    elif args.task == '1-1':
        first = args.continuous
        second = args.discrete
        node_feat = first + second
    elif args.task == '1-2':
        first = args.discrete
        second = args.continuous
        node_feat = first + second
    else:
        node_feat = list(df_X.columns)
        init_feat = []
    ...
    # add
    edge_start, edge_end = create_edge(df_X[node_feat])
    edge_index = torch.tensor([edge_start, edge_end], dtype=int)
    edge_attr = torch.tensor(create_edge_attr(df_X[node_feat]), dtype=torch.float)
    node_init = create_node(df_X[node_feat], node_mode, df_X[init_feat]) 
    ...
    # add
    if args.task == '1-1' or args.task == '1-2':
        train_edge_mask = get_known_mask(train_edge_prob, int(nrow * len(first)))
        train_edge_mask = train_edge_mask.reshape((nrow, len(first)))
        train_edge_mask = torch.cat((train_edge_mask, torch.ones(nrow, len(second)).bool()), dim = 1).reshape(nrow * ncol)
        double_train_edge_mask = torch.cat((train_edge_mask, train_edge_mask), dim=0)
    ...
    train_labels = train_edge_attr[:int(train_edge_attr.shape[0]/2),0]     # O_i -> F_i 的edge
    test_edge_index, test_edge_attr = mask_edge(edge_index, edge_attr,
                                                ~double_train_edge_mask, True)
    test_labels = test_edge_attr[:int(test_edge_attr.shape[0]/2),0]     # O_i -> F_i 的edge
    ...
    if hasattr(args,'impute_ce') and args.impute_ce:
        data.class_values = int(df_X[args.discrete].max().max()) + 1

    if args.data_type == 'cls':
        data.class_y = df_y.max() + 1
...
def load_data(args):
    ...
    if not hasattr(args,'split_sample'):
        args.split_sample = 0
    data = get_data(df_X, df_y, args.node_mode, args.train_edge, args.split_sample, args.split_by, args.train_y, args, args.seed)
    return data
